export default class Character {
    constructor({ name, image, gender, species, status, episode_names }) {
      this.name = name
      this.image = image
      this.gender = gender
      this.species = species
      this.status = status
      this.episode_names = episode_names
    ...
    buildDescription() {
      return `
        <div class="character-labels">
          ...
          <h3 class="character-label">Episode Names: ${this.episode_names}</h3>
        </div>
      `
    }
    render() {
